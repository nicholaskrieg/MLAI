...
def minimax(state, player, max_depth):
    ...
    minimaxSuccessors = [successor(state, move, player) for move in legal_moves(state, player)]

    if player == "#":
        return max(minimax(s, 'O', max_depth - 1) for s in minimaxSuccessors)
    elif player == "O":
        return min(minimax(s, '#', max_depth - 1) for s in minimaxSuccessors)


def best_move(state, player, max_depth):
    """Returns player's best move. max_depth, which must be at least 1, gives the search depth."""

    legalMoves = legal_moves(state, player)

    if player == "#":
        return max(legalMoves, key=lambda m: minimax(successor(state, m, player), 'O', max_depth - 1))
    elif player == "O":
        return min(legalMoves, key=lambda m: minimax(successor(state, m, player), '#', max_depth - 1))
    ...
    while not game_over(game):
        ...
        if not game_over(game):
            while True:
                print('O to play')
                print(prettify(game))
                m = input('Enter row and column (0-7, separated by a space): ')
                if m != 'pass':
